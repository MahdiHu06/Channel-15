...
FRESULT log_data_csv(const char *filename, int loggedData[10][3]) {
    ...
    // Buffer for previous rows (max 10)
    int prev[10][3];
    int prev_count = 0;

    printf("log: trying to open existing file for read\n");
    ...
    if (fr == FR_OK) {
        while (f_gets(line, sizeof(line), &fil)) {
            ...
            if (sscanf(line, "%d,%d,%d", &a, &b, &c) == 3) {
                if (prev_count < 10) {
                    ...
                    // Shift left to keep only newest 10
                    memmove(prev, prev + 1, 9 * sizeof(prev[0]));
                    prev[9][0] = a;
                    prev[9][1] = b;
                    prev[9][2] = c;
                }
        ...
        f_close(&fil);
        printf("log: previous data loaded\n");
    }
    else if (fr != FR_NO_FILE) {
        print_error(fr, "f_open_read");
        spi1_unlock();
        return fr;
    }
    else {
        printf("log: file does not exist (starting fresh)\n");
    }
    ...
    printf("log: now writing data\n");



    // // Write previous rows if any
...
void init_spi_sdcard() {
    ...
    // gpio_set_function(SD_SCK, GPIO_FUNC_SPI);        // Handled by display
    // gpio_set_function(SD_MOSI, GPIO_FUNC_SPI);       // Handled by display
...
void disable_sdcard() {
    ...
    uint8_t dummy = 0xFF;
    spi_write_blocking(SD_SPI_INST, &dummy, 1);
}
...
void init_sdcard_io() {
    // DON'T call init_spi_sdcard() here - SPI1 is already initialized by LCD
    // Just set the low speed for SD card initialization
    spi_set_baudrate(SD_SPI_INST, 400 * 1000);
    spi_set_format(SD_SPI_INST, 8, SPI_CPOL_0, SPI_CPHA_0, SPI_MSB_FIRST);
    
    // Ensure MISO is configured for SPI
    gpio_set_function(SD_MISO, GPIO_FUNC_SPI);
    
    // Ensure CS is configured
    gpio_init(SD_CS);
    gpio_set_dir(SD_CS, GPIO_OUT);
    gpio_put(SD_CS, 1);
...
FRESULT save_measurement(const char *filename, int temp, int pressure, int humidity) {
    ...
    fr = f_open(&fil, filename, FA_READ);
    if (fr == FR_OK) {
        while (f_gets(line, sizeof(line), &fil) && count < 10) {
            int a, b, c;
            if (sscanf(line, "%d,%d,%d", &a, &b, &c) == 3) {
                data[count][0] = a;
                data[count][1] = b;
    ...
    newData[0][0] = temp;
    newData[0][1] = pressure;
    newData[0][2] = humidity;
    
    for (int i = 0; i < 9; i++) {
        if (i < count) {
            newData[i + 1][0] = data[i][0];
            newData[i + 1][1] = data[i][1];
            newData[i + 1][2] = data[i][2];
        } else {
            newData[i + 1][0] = 0;
            newData[i + 1][1] = 0;
            newData[i + 1][2] = 0;
